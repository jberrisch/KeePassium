...
/// Common parts of Argon2d and Argon2id KDFs
class AbstractArgon2KDF {
    ...
    fileprivate let maxIterations: UInt64 = UInt64(UInt32.max) // limited to UInt32
    ...
    fileprivate let defaultMemory: UInt64      = 1024 * 1024 // 1 MiB
    ...
    /// Converts `KDFParams` into an internal struct, also checking whether given parameters are specified and in valid ranges.
    /// - Throws: `CryptoError.invalidKDFParam`
    fileprivate func getParams(_ params: KDFParams) throws -> Argon2.Params {
        ...
        return Argon2.Params(
            ...
            memoryKiB: UInt32(memory / UInt64(1024)), // Argon2.hash takes memory in KiB
            iterations: UInt32(iterations64), // will crash in case of int overflow
            version: version
        )
    }
    ...
    func transform(key: SecureByteArray, params: KDFParams) throws -> SecureByteArray {
        ...
        let hashingParams = try getParams(params) // throws `CryptoError.invalidKDFParam`
