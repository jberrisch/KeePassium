...
public class NavigationRouter: NSObject {
    ...
    public func pop(animated: Bool) {
        ...
        if isLastVC {
            // cannot pop the last VC, so dismiss the whole navVC instead
            navigationController.dismiss(animated: animated, completion: nil)
            triggerAndRemovePopHandler(for: navigationController.topViewController!) // safe to unwrap
        } else {
...
extension NavigationRouter: UIAdaptivePresentationControllerDelegate {
    public func presentationControllerDidDismiss(_ presentationController: UIPresentationController) {
        ...
        pop(animated: false) // dismiss the popover
    }
}
