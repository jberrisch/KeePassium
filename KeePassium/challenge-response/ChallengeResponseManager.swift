...
class ChallengeResponseManager {
    ...
    private var mfiKeyActionSheetView: MFIKeyActionSheetView? // owned, strong
    ...
    private func cancelMFISession() {
        ...
        if accessorySession.sessionState == .opening || accessorySession.sessionState == .open {
            ...
            // The key is waiting for a touch,
            // the sessing cannot be stopped, but we can cancel the pending commands.
            // https://github.com/Yubico/yubikit-ios/issues/34#issuecomment-594216870
